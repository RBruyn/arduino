...
Repetidor para Sendor X:  byte 1
Sensor X para Repetidor:  byte 2
Repetidor para Base:      byte 3
 */
...
// SEMPRE VAI RECEBER O PACOTE
  ...
  if (Serial.available() == 52)   // Recebe pacote que entrou pela USB
  {
	...
	Pacote[i] = Serial.read();  // Faz a leitura dos bytes que estão no buffer da serial
      ...
      // ===================================================  Chega pacote da base para repetidor - Indo
      if ( Pacote[8] == 20)
        {
      ...
      //  ==================================================  Pacote do repetidor para o sensor X - Indo
      ...
      // ===================================================  Chega pacote do sensor para o repetidor - Voltando
      if ( Pacote[8] == 20)
        {
          // Manda do sensor X para o repetidor
          Pacote[10] = Pacote[8]; // O sensor está enviando          
          Pacote[11] = Pacote[9]; // Vai para a base
          
          // Gera a RSSI - Range de -137 <- 0 -> -137,5
          Pacote[3] = random(-126,129);          
        }

      // ===================================================  Chega pacote do repetidor para a base - Voltando
      // Gera a RSSI - Range de -137 <- 0 -> -137,5
      Pacote[2] = random(-126,129);
